...
def genmcard():
    ...
    rs = requests.post(sitex , headers=headers , cookies=cookies,data=payload)
    data = json.loads(rs.text)
    open("generated_cardby@Extra_Earnn.txt","w").write("")
    for i in range(1,10):
    ...
    return (gn + "[$] The operation has been success\n[+] Saved File as generated_cardby@Extra_Earnn.txt" + cv)
...
if opr == "1":
    ...
    print ("Saved File as bin_generatedby@Extra_Earnn.txt !")
    print (mag + "[$] Telegram Channel : @EXTRA_EARNN " + cv)